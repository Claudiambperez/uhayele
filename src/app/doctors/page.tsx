        ...
        <ul className='flex flex-col gap-4 text-sm text-gray-500 px-6 '>
         {
          services.map((doctor,index) => (
            <li onClick={() => speciality=== doctor.speciality ? router.push('/doctors') : router.push(`/doctors?speciality=${doctor.speciality}`) } className={`w-[96vw] sm:w-auto pl-3 py-2 pr-16 border border-gray-300 rounded cursor-pointer transition-all ${speciality === doctor.speciality ? 'bg-sex text-white' : ''}`} key={index}>{doctor.speciality} </li>
          ))
         }
        </ul>
        <div className='  '>
           <div className='  grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 4 gap-x-8 gap-y-[-4px] '>
          {
            filterDoc.map((doctor,index) => (
                      <div onClick={() => router.push(`/appointment/${doctor.id}`)} 
                      className=' overflow-hidden  hover:translate-y-[10px] cursor-pointer group relative flex flex-col my-6  shadow-sm border border-slate-200 rounded-lg  max-w-68 hover:shadow-lg transition-shadow duration-300 ' key={index}>
